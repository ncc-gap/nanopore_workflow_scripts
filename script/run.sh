...
mkdir -p "split/nanopolish/"${PARAM_CONTL}
mkdir -p "vep/"${PARAM_CONTL}
mkdir -p "whatshap/"${PARAM_CONTL}
mkdir -p "nanopolish/"${PARAM_CONTL}
...
do
    ...
    if [${PARAM_TUMOR} != ${PARAM_CONTL} ]
    then
        qsub ${QSUB_CONTL} -N ${PARAM_CONTL}_nanopolish -hold_jid ${PARAM_CONTL}_minimap2 script/singularity_nanopolish.sh ${PARAM_CONTL} ${CHR} ${PARAM_CONTL_FASTQ} ${PARAM_CONTL_FAST5}
    fi
done
...
if [${PARAM_TUMOR} != ${PARAM_CONTL} ]
then
    qsub ${QSUB_CONTL} -N ${PARAM_CONTL}_nanopolish_merge -hold_jid ${PARAM_CONTL}_nanopolish script/singularity_nanopolish_merge.sh ${PARAM_CONTL}
fi
